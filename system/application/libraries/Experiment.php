...
class Experiment {
    ...
    public function get_phase_one_permutated_rounds(){
        ...
        $extracted_parent_rounds = array();
        foreach($parent_rounds as $parent_round){
            $extracted_parent_rounds[] = $parent_round['id'];
        }

        shuffle(&$extracted_parent_rounds);

        $rounds = array();
        foreach($extracted_parent_rounds as $parent_round_id){
            $rounds[] = $parent_round_id;
        }

        foreach($child_rounds as $child_round){
            $extracted_child_rounds[] = $child_round['id'];
        }
        
         shuffle(&$extracted_child_rounds);
         
         foreach($extracted_child_rounds as $child_round_id){
         	$rounds[] = $child_round_id;
         }
        ...
        foreach($extracted_single_rounds as $single_round_id){
            $rounds[] = $single_round_id;
        }
        
        //print_r(count($rounds));
        //print_r($rounds);
        ...
        public function is_bundle_conditionally_unavailable($round_id = 0, $subject_id = 0, $bundle = array(), $previously_selected_bundle){
        	...
        	if(!isset($previously_selected_bundle['bundle_id']) || empty($previously_selected_bundle['bundle_id'])){
		         ...
		         */
                if($round_id == 57 || $round_id == 58 || $round_id == 59){
                ...
                if($round_id == 21){
                	switch((int)$bundle['id']){
                		case 62:
                			if($previously_selected_bundle_id == 54 || $previously_selected_bundle_id == 53){
                				return true;
                			}
                			break;
                		case 64:
                			if($previously_selected_bundle_id == 56 || $previously_selected_bundle_id == 55){
                				return true;
                			}
                			break;
                	}
                ...
                elseif($round_id == 24){
                	switch((int)$bundle['id']){
                		case 74:
                			if($previously_selected_bundle_id == 66 || $previously_selected_bundle_id == 65){
                				return true;
                			}
                			break;
                		case 76:
                			if($previously_selected_bundle_id == 68 || $previously_selected_bundle_id == 67){
                				return true;
                			}
                			break;
                	}
	...
	public function get_parent_round_id($round, $round_id){
        	if($round['conditional_bundles']){
                if($round_id == 57 || $round_id == 58 || $round_id == 59){   // one parent multiple children
                    $round_id = 47;
                }
                elseif(!empty($round['parent_id']) && $round_id != 21 && $round_id != 24){ // multiple conditions set on bundles
    ...
    public function get_previously_selected_bundle($round, $round_id, $subject_id){
        	if($round['conditional_bundles']){
                if($round_id == 57 || $round_id == 58 || $round_id == 59){   // one parent multiple children
                    return $this->get_phase_one_selected_bundle(47, $subject_id);
                }
                elseif($round_id == 21){
                	return $this->get_phase_one_selected_bundle(19, $subject_id);
                }
                elseif($round_id == 24){
                	return $this->get_phase_one_selected_bundle(22, $subject_id);
                }
                elseif(!empty($round['parent_id']) && $round_id != 21 && $round_id != 24){ // multiple conditions set on bundles
